...
function updateProgress(){const done=records.filter(r=>labels[r.record_id]?.truth_label).length; document.getElementById('progress').textContent=`${done}/${records.length} labeled`}
