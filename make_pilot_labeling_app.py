...
function defaultLabel(r){if(!r.images.length){return {truth_label:'cannot_determine',image_quality:'unusable',occlusion:'unknown',detectable_warning:'cannot_determine',labeler:'evidence_gate',notes:'No target view passed the frozen evidence gate.',label_timestamp:'',label_source:'automatic_evidence_gate'}}return {truth_label:'',image_quality:'',occlusion:'',detectable_warning:'',labeler:'',notes:'',label_timestamp:'',label_source:'human_review'}}
function current(){const r=records[index]; labels[r.record_id] ||= defaultLabel(r); return [r,labels[r.record_id]]}
for(const r of records){if(!labels[r.record_id]||(!r.images.length&&!isComplete(labels[r.record_id])))labels[r.record_id]=defaultLabel(r)}
...
document.querySelectorAll('[data-truth]').forEach(b=>b.onclick=()=>{l.truth_label=b.dataset.truth;l.label_timestamp=new Date().toISOString();l.label_source='human_review';save();render()});
document.querySelectorAll('[data-field]').forEach(el=>el.onchange=()=>{l[el.dataset.field]=el.value;if(r.images.length)l.label_source='human_review';save()}); document.getElementById('previous').disabled=index===0;document.getElementById('next').disabled=index===records.length-1;updateProgress()}
